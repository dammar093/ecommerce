            ...
            >
              <div className='flex justify-between flex-wrap gap-2 mt-4'>
                ...
                <div className='md:w-[20%] w-full'>
                  <div className='w-full h-10 rounded-md bg-slate-200 text-gray-600'>
                    <select className='w-full h-full bg-transparent capitalize'  {...register('category', {
                      required: 'Category is required !',
                    })}>
                      <option className='text-gray-600 text-md capitalize' value="">Select Category</option>
                      {
                        categories.map(item => (

                          <option className='text-gray-600 text-md capitalize' value={item.title} key={item._id}>{item.title}</option>
              ...
              <div className='flex justify-between flex-wrap gap-2 mt-2'>
                <div className='md:w-[70%] w-full'>
                  <div className='w-full h-10 rounded-md bg-slate-200 text-gray-600 flex items-center'>
                    ...
                    />
                    <Button className={`bg-[#AE56EF] h-full hover:bg-[#8d48be] text-[#f3f3f3] p-1 rounded `} type="button"
                      onClick={addColor}
                    >
                      <div className='flex justify-center text-md items-center gap-1'>    <span><IoAddCircleOutline /></span>
                        <span>Add</span>
                      </div>
                    </Button>
                  </div>
                  <div className='flex gap-1 items-center'>
                    {
                      colors.map(clr => (
                        <div className='w-[60px] p-1 border border-gray-900 rounded-full flex justify-between items-center' key={clr}>
                          <div className={`w-[20px] h-[20px] rounded-full`} style={{ backgroundColor: clr }}>
                          </div>
                          <div>
                ...
                <div className='md:w-[28%] w-full'>
                  <div className='w-full h-10 rounded-md bg-slate-200 text-gray-600 flex items-center'>
                    {/* <Input
                      type="text"
                      className="w-full h-full bg-transparent text-gray-600 rounded outline-none px-2"
                      placeholder="Enter sizes of products"
                      {...register('sizes')}
                      value={size}
                      onChange={(e) => setSize(e.target.value)}
                    /> */}
                    <select className='w-full h-full uppercase bg-transparent'
                      value={size}
                      onChange={(e) => setSize(e.target.value)}
                    >
                  ...
                  <div className='flex gap-1 items-center uppercase mt-2'>
                    {
                      sizes.map(sz => (
                        <div className='w-[60px] p-1 border border-gray-900 rounded-full flex justify-between items-center' key={sz}>
                ...
                >
                  <div>
                    ...
                    <div className='w-full text-center'>
                      ...
                      <span className='text-center text-[#AE56EF] cursor-pointer'
                ...
                <div className='flex gap-2 mt-2'>
                  {imageUrls?.map((url, index) => (
                    <div className='w-[100px] h-[100px] relative'>
                      <img className='object-cover h-full w-full' key={index} src={url} alt={`Image ${index}`} />
                      <span className='float-right text-md flex items-center justify-center text-[#f3f3f3] cursor-pointer absolute top-0 right-0 w-[20px] h-[20px] bg-red-400 rounded-full' onClick={() => removeImage(url)}> <RxCross2 /></span>
                    </div>
                  ))}
